...
public static class Core
{
  ...
  private const int DirectorySeparatorCharCode = (int)CharacterCodes.Slash;

  // from core.ts

  public static int BinarySearch(int[] array, int value, Func<int, int, int>? comparer = null,
    int? offset = null)
  {
  ...
  public static string NormalizePath(string path)
  {
    ...
    if (!normalized.Any()) return root;
    var joinedParts =
      root + string.Join(DirectorySeparator.ToString(), normalized); //.join(directorySeparator);
  ...
  private static int GetRootLength(string path)
  {
    ...
    {
      if (path.charCodeAt(1) != (int)CharacterCodes.Slash) return 1;
    ...
    {
      if (path.charCodeAt(2) == (int)CharacterCodes.Slash) return 3;
      return 2;
    }

    if (path.LastIndexOf("file:///", 0, StringComparison.Ordinal) == 0) return "file:///".Length;
    var idx = path.IndexOf("://", StringComparison.Ordinal);
    if (idx != -1) return idx + "://".Length;
    return 0;
  }
  ...
  private static bool PathEndsWithDirectorySeparator(string path)
  {
  ...
  public static Diagnostic CreateFileDiagnostic(SourceFile file, int start, int length,
    ...
    params string[] arguments)
  {
    ...
    Debug.Assert(start <= file.Text.Length,
      $"start must be within the bounds of the file. {start} > {file.Text.Length}");
    Debug.Assert(end <= file.Text.Length,
      $"end must be the bounds of the file. {end} > {file.Text.Length}");
  ...
  private static string GetLocaleSpecificMessage(DiagnosticMessage message)
  {
    return
