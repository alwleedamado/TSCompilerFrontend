...
public class JsDocParser(Parser parser)
{
  ...
  private SyntaxKind CurrentToken
  {
    ...
    set => Parser.CurrentToken = value;
  }

  private bool ParseErrorBeforeNextFinishedNode
  {
    get => Parser.ParseErrorBeforeNextFinishedNode;
    set => Parser.ParseErrorBeforeNextFinishedNode = value;
  }

  private List<Diagnostic> ParseDiagnostics
  {
    get => Parser.ParseDiagnostics;
    set => Parser.ParseDiagnostics = value;
  }
  ...
  private JsDocTypeExpression ParseJsDocTypeExpression()
  {
  ...
  private IJsDocType ParseJsDocTopLevelType()
  {
    ...
    if (Token() != SyntaxKind.EqualsToken) return type;
  ...
  private IJsDocType ParseJsDocType()
  {
  ...
  private IJsDocType ParseBasicTypeExpression()
  {
    switch (Token())
    {
      case SyntaxKind.AsteriskToken:
  ...
  private JsDocThisType ParseJsDocThisType()
  {
  ...
  private JsDocConstructorType ParseJsDocConstructorType()
  {
  ...
  private JsDocVariadicType ParseJsDocVariadicType()
  {
  ...
  private JsDocFunctionType ParseJsDocFunctionType()
  {
    var result = new JsDocFunctionType();
    NextToken();
    ParseExpected(SyntaxKind.OpenParenToken);
    result.Parameters =
      Parser.ParseDelimitedList(ParsingContext.JSDocFunctionParameters, ParseJsDocParameter);
    CheckForTrailingComma(result.Parameters);
    ParseExpected(SyntaxKind.CloseParenToken);
    if (Token() != SyntaxKind.ColonToken) return FinishNode(result);
  ...
  private ParameterDeclaration ParseJsDocParameter()
  {
    var parameter = new ParameterDeclaration { Type = ParseJsDocType() };
    if (ParseOptional(SyntaxKind.EqualsToken)) parameter.QuestionToken = new QuestionToken();
    return FinishNode(parameter);
  }

  private JsDocTypeReference ParseJsDocTypeReference()
  {
  ...
  private NodeArray<IJsDocType> ParseTypeArguments()
  {
    NextToken();
  ...
  private void CheckForEmptyTypeArgumentList<T>(NodeArray<T> typeArguments)
  {
  ...
  private QualifiedName ParseQualifiedName(IEntityName left)
  {
  ...
  private JsDocRecordType ParseJsDocRecordType()
  {
    var result = new JsDocRecordType { Literal = ParseTypeLiteral() };
    return FinishNode(result);
  }

  private JsDocNonNullableType ParseJsDocNonNullableType()
  {
  ...
  private JsDocTupleType ParseJsDocTupleType()
  {
  ...
  private void CheckForTrailingComma<T>(NodeArray<T> list)
  {
  ...
  private JsDocUnionType ParseJsDocUnionType()
  {
  ...
  private NodeArray<IJsDocType> ParseJsDocTypeList(IJsDocType firstType)
  {
    var types = Parser.CreateList<IJsDocType>();
    types.Add(firstType);
    types.Pos = firstType.Pos;
    while (ParseOptional(SyntaxKind.BarToken)) types.Add(ParseJsDocType());
    types.End = Scanner.GetStartPos();
    return types;
  }

  private JsDocAllType ParseJsDocAllType()
  {
  ...
  private JsDocLiteralType ParseJsDocLiteralType()
  {
    var result = new JsDocLiteralType { Literal = Parser.ParseLiteralTypeNode() };
    return FinishNode(result);
  }

  private JsDocType ParseJsDocUnknownOrNullableType()
  {
    NextToken();
    ...
    int length)
  {
    ...
    Parser.SourceFile = new SourceFile
      { LanguageVariant = LanguageVariant.Standard, Text = content };
  ...
  public JsDoc ParseJsDocComment(INode parent, int? start, int? length)
  {
    ...
    if (comment != null) comment.Parent = parent;
  ...
  private JsDoc ParseJsDocCommentWorker(int? start = null, int? length = null)
  {
    ...
    Scanner.ScanRange(start + 3, (length ?? 0) - 5, () =>
      {
        ...
        NextJsDocToken();
        while (Token() == SyntaxKind.WhitespaceTrivia) NextJsDocToken();
    ...
    List<string?> ParseTagComments(int indent)
    {
      ...
      while (Token() != SyntaxKind.AtToken && Token() != SyntaxKind.EndOfFileToken)
      {
        ...
        if (Token() == SyntaxKind.AtToken) break;
    ...
    JsDocTypeExpression TryParseTypeExpression()
    {
      ...
      {
        SkipWhitespace();
        if (Token() != SyntaxKind.OpenBraceToken) return null;
    ...
    JsDocParameterTag ParseParamTag(AtToken atToken, Identifier tagName)
    {
      ...
      if ((OpenBracketToken)ParseOptionalToken<OpenBracketToken>(SyntaxKind.OpenBracketToken) !=
          null)
      {
      ...
      if (typeExpression != null)
        ...
        preName = name;
      if (typeExpression == null) typeExpression = TryParseTypeExpression();
    ...
    JsDocReturnTag ParseReturnTag(AtToken atToken, Identifier tagName)
    {
      if (tags.Any(t => t.Kind == SyntaxKind.JsDocReturnTag))
        ParseErrorAtPosition(tagName.Pos ?? 0, Scanner.GetTokenPos() - (tagName.Pos ?? 0),
    ...
    IJsDocTag ParseTypedefTag(AtToken atToken, Identifier tagName)
    {
      ...
      if (typedefTag.FullName != null)
      {
        ...
        while (true)
        {
          if (rightNode.Kind == SyntaxKind.Identifier ||
              (rightNode as JsDocNamespaceDeclaration)?.Body == null)
          {
            typedefTag.Name = rightNode.Kind == SyntaxKind.Identifier
              ? rightNode
              : (rightNode as JsDocTypedefTag)?.Name;
            break;
          }
      ...
      if (typeExpression != null)
      {
        if (typeExpression.Type.Kind == SyntaxKind.JsDocTypeReference)
        {
          ...
          if (jsDocTypeReference.Name.Kind == SyntaxKind.Identifier)
          {
            ...
            if (name?.Text == "Object") typedefTag.JsDocTypeLiteral = ScanChildTags();
          }
        }
    ...
    bool TryParseChildTag(JsDocTypeLiteral parentTag)
    {
      ...
      {
        case "type":
          if (parentTag.JsDocTypeTag != null) return false;
  ...
  private void FixupParentReferences(INode rootNode)
  {
  ...
  private T TryParse<T>(Func<T> callback)
  {
    return Parser.TryParse<T>(callback);
  }

  private bool ParseExpected(SyntaxKind kind, DiagnosticMessage diagnosticMessage = null,
    bool shouldAdvance = true)
  {
    return Parser.ParseExpected(kind, diagnosticMessage, shouldAdvance);
  }

  private bool ParseOptional(SyntaxKind t)
  {
    return Parser.ParseOptional(t);
  }

  private INode ParseOptionalToken<T>(SyntaxKind t) where T : Node
  {
    return Parser.ParseOptionalToken<T>(t);
  }

  private T ParseTokenNode<T>() where T : Node
  {
    return Parser.ParseTokenNode<T>(Token());
  }

  private NodeArray<T> CreateList<T>(T[] elements = null, int? pos = null)
  {
    return Parser.CreateList<T>(elements, pos);
  }
  ...
  private NodeArray<T> ParseDelimitedList<T>(ParsingContext kind, Func<T> parseElement,
    bool? considerSemicolonAsDelimiter = null) where T : INode
  {
    return Parser.ParseDelimitedList<T>(kind, parseElement, considerSemicolonAsDelimiter);
  }
