...
using static TSCompilerFrontend.Parser.Core;
using static TSCompilerFrontend.Parser.Scanner;
using static TSCompilerFrontend.Parser.Ts;
using static TSCompilerFrontend.Parser.Utilities;

namespace TSCompilerFrontend.Parser;

public class Parser
{
  private const NodeFlags DisallowInAndDecoratorContext =
    NodeFlags.DisallowInContext | NodeFlags.DecoratorContext;
  private NodeFlags _contextFlags;
  private int _identifierCount;
  private List<string?> _identifiers;
  private readonly JsDocParser _jsDocParser;
  private int _nodeCount;
  public List<Diagnostic> ParseDiagnostics;
  public bool ParseErrorBeforeNextFinishedNode;
  private int _parsingContext;

  public readonly Scanner Scanner = new Scanner(ScriptTarget.Latest, true, LanguageVariant.Standard, null);
  ...
  public Parser()
  {
    _jsDocParser = new JsDocParser(this);
  }

  public SourceFile ParseSourceFile(string fileName, string? sourceText,
    ScriptTarget languageVersion, /*IncrementalParser.SyntaxCursor*/object? syntaxCursor,
    bool setParentNodes,
  ...
  private LanguageVariant GetLanguageVariant(ScriptKind scriptKind)
  {
  ...
  public void InitializeState(string? _sourceText,
    ScriptTarget languageVersion, /*IncrementalParser.SyntaxCursor*/
    object? syntaxCursor, ScriptKind scriptKind)
  {
    ...
    _identifierCount = 0;
    _nodeCount = 0;
    _contextFlags = scriptKind == ScriptKind.Js || scriptKind == ScriptKind.Jsx
  ...
  private SourceFile ParseSourceFileWorker(string fileName, ScriptTarget languageVersion,
    bool setParentNodes,
    ScriptKind scriptKind)
  {
    SourceFile = CreateSourceFile(fileName, languageVersion, scriptKind);
    SourceFile.Flags = _contextFlags;
    // Prime the scanner.
    NextToken();
    ...
    SourceFile.NodeCount = _nodeCount;
    SourceFile.IdentifierCount = _identifierCount;
    SourceFile.Identifiers = _identifiers;
    SourceFile.ParseDiagnostics = ParseDiagnostics;
    if (setParentNodes) FixupParentReferences(SourceFile);
    return SourceFile;
  }

  private T AddJsDocComment<T>(T node) where T : INode
  {
    var comments = GetJsDocCommentRanges(node, SourceFile.Text);
    if (comments.Any())
      foreach (var comment in comments)
      {
        var jsDoc = _jsDocParser.ParseJsDocComment(node, comment.Pos, comment.End - comment.Pos);
        if (jsDoc == null) continue;
        if (node.JsDoc == null) node.JsDoc = new List<JsDoc>();
  ...
  public SourceFile CreateSourceFile(string fileName, ScriptTarget languageVersion,
    ScriptKind scriptKind)
  {
  ...
  {
    var contextFlagsToClear = context & _contextFlags;
  ...
  private T DoInsideOfContext<T>(NodeFlags context, Func<T> func)
  {
    var contextFlagsToSet = context & ~_contextFlags;
  ...
  public void ParseErrorAtPosition(int start, int length, DiagnosticMessage message,
    object? arg0 = null)
  {
  ...
  private void ScanError(DiagnosticMessage message, int? length = null)
  {
  ...
  private SyntaxKind ReScanTemplateToken()
  {
    CurrentToken = Scanner.ReScanTemplateToken();
    return CurrentToken;
  }

  private SyntaxKind ScanJsxIdentifier()
  {
    CurrentToken = Scanner.ScanJsxIdentifier();
    return CurrentToken;
  }

  private SyntaxKind ScanJsxText()
  {
    CurrentToken = Scanner.ScanJsxToken();
    return CurrentToken;
  }

  private SyntaxKind ScanJsxAttributeValue()
  {
  ...
  private T SpeculationHelper<T>(Func<T> callback, bool isLookAhead)
  {
    ...
    var saveContextFlags = _contextFlags;
    var result = isLookAhead
      ? Scanner.LookAhead(callback)
      : Scanner.TryScan(callback);

    Debug.Assert(saveContextFlags == _contextFlags);
  ...
  public bool ParseExpected(SyntaxKind kind, DiagnosticMessage diagnosticMessage = null,
  ...
  {
    if (Token() == t) return ParseTokenNode<T>(Token());

    return null;
  }

  private Node ParseExpectedToken<T>(SyntaxKind t, bool reportAtCurrentPosition,
    DiagnosticMessage diagnosticMessage, object? arg0 = null) where T : Node
  {
  ...
  {
    var array = elements == null
      ? new NodeArray<T>()
      : new NodeArray<T>(elements); // (List<T>)(elements || []);
    if (!(pos >= 0)) pos = GetNodePos();
  ...
  public T FinishNode<T>(T node, int? end = null) where T : INode
  {
    ...
    if (_contextFlags != NodeFlags.None) node.Flags |= _contextFlags;
  ...
  private Node CreateMissingNode<T>(SyntaxKind kind, bool reportAtCurrentPosition,
    DiagnosticMessage diagnosticMessage = null, object? arg0 = null) where T : Node
  {
  ...
  private string? InternIdentifier(string? text)
  {
    ...
    if (!_identifiers.Contains(text)) // identifier == null)
      _identifiers.Add(text); //.set(text, identifier = text);
    return text; // identifier;
  }

  private Identifier? CreateIdentifier(bool isIdentifier,
    DiagnosticMessage diagnosticMessage = null)
  {
    ...
    if (isIdentifier)
    {
      ...
      if (Token() != SyntaxKind.Identifier) node.OriginalKeywordKind = Token();
      node.Text = InternIdentifier(Scanner.GetTokenValue());
      NextToken();
      return FinishNode(node);
    }

    return (Identifier)CreateMissingNode<Identifier>(
  ...
  private IPropertyName? ParsePropertyNameWorker(bool allowComputedPropertyNames)
  {
    if (Token() == SyntaxKind.StringLiteral || Token() == SyntaxKind.NumericLiteral)
    {
      var le = ParseLiteralNode( /*internName*/ true);
      if (le is StringLiteral) return (StringLiteral)le;
      if (le is NumericLiteral) return (NumericLiteral)le;
  ...
  private IPropertyName? ParsePropertyName()
  {
  ...
  private bool IsSimplePropertyName()
  {
    return Token() == SyntaxKind.StringLiteral || Token() == SyntaxKind.NumericLiteral ||
  ...
  private bool NextTokenCanFollowModifier()
  {
    ...
    if (Token() == SyntaxKind.DefaultKeyword) return NextTokenIsClassOrFunctionOrAsync();
  ...
  private bool NextTokenIsClassOrFunctionOrAsync()
  {
    NextToken();
  ...
  private bool IsListElement(ParsingContext parsingContext, bool inErrorRecovery)
  {
    ...
    switch (parsingContext)
    {
      ...
      case TsTypes.ParsingContext.JSDocTupleTypes:
        return _jsDocParser.IsJsDocType();
  ...
  private bool IsListTerminator(ParsingContext kind)
  {
    ...
    switch (kind)
    {
      ...
      case TsTypes.ParsingContext.JSDocFunctionParameters:
        return Token() == SyntaxKind.CloseParenToken || Token() == SyntaxKind.ColonToken ||
  ...
  private bool IsInSomeParsingContext()
  {
    ...
    foreach (ParsingContext kind in Enum.GetValues(typeof(ParsingContext)))
      if ((_parsingContext & (1 << (int)kind)) != 0)
  ...
  private NodeArray<T> ParseList<T>(ParsingContext kind, Func<T> parseElement) where T : INode
  {
    var saveParsingContext = _parsingContext;

    _parsingContext |= 1 << (int)kind;
    ...
    _parsingContext = saveParsingContext;
    return result;
  }

  private NodeArray<T> ParseList2<T>(ParsingContext kind, Func<T> parseElement) where T : INode
  {
    var saveParsingContext = _parsingContext;
  ...
  private T ParseListElement<T>(ParsingContext parsingContext, Func<T> parseElement)
    where T : INode
  {
    var node = CurrentNode(parsingContext);
    if (node != null) return (T)ConsumeNode(node);
    return parseElement();
  }

  private T ParseListElement2<T>(ParsingContext parsingContext, Func<T> parseElement)
    where T : INode
  {
  ...
  private Node CurrentNode(ParsingContext parsingContext)
  {
    if (ParseErrorBeforeNextFinishedNode) return null;
    return null;
  ...
  private INode CurrentNode2(ParsingContext parsingContext)
  {
    if (ParseErrorBeforeNextFinishedNode) return null;
    return null;
  }
  ...
  private bool IsReusableClassMember(Node node)
  {
    if (node != null)
      switch (node.Kind)
      {
        ...
        case SyntaxKind.MethodDeclaration:
          ...
          var nameIsConstructor = methodDeclaration.Name.Kind == SyntaxKind.Identifier &&
                                  ((Identifier)methodDeclaration.Name).OriginalKeywordKind ==
  ...
  private bool IsReusableSwitchClause(Node node)
  {
    if (node != null)
      switch (node.Kind)
      {
  ...
  private bool IsReusableStatement(Node node)
  {
    if (node != null)
      switch (node.Kind)
      {
  ...
  private bool IsReusableEnumMember(Node node)
  {
    return node.Kind == SyntaxKind.EnumMember;
  }

  private bool IsReusableTypeMember(Node node)
  {
    if (node != null)
      switch (node.Kind)
      {
  ...
  private bool IsReusableVariableDeclaration(Node node)
  {
    if (node.Kind != SyntaxKind.VariableDeclaration) return false;
    var variableDeclarator = (VariableDeclaration)node;
    return variableDeclarator.Initializer == null;
  }

  private bool IsReusableParameter(Node node)
  {
  ...
  public NodeArray<T> ParseDelimitedList<T>(ParsingContext kind, Func<T> parseElement,
    bool? considerSemicolonAsDelimiter = null) where T : INode
  {
    var saveParsingContext = _parsingContext;
    _parsingContext |= 1 << (int)kind;
    var result = CreateList<T>();
    ...
    while (true)
    {
      if (IsListElement(kind, /*inErrorRecovery*/ false))
      {
        ...
        if (considerSemicolonAsDelimiter == true && Token() == SyntaxKind.SemicolonToken &&
            !Scanner.HasPrecedingLineBreak()) NextToken();
        continue;
      }
  ...
  private NodeArray<T> CreateMissingList<T>() where T : INode
  {
    return CreateList<T>();
  }

  private NodeArray<T> ParseBracketedList<T>(ParsingContext kind, Func<T> parseElement,
    SyntaxKind open,
    SyntaxKind close) where T : INode
  {
  ...
  private IEntityName? ParseEntityName(bool allowReservedWords,
    DiagnosticMessage diagnosticMessage = null)
  {
  ...
  private TemplateExpression? ParseTemplateExpression()
  {
    var template = new TemplateExpression { Pos = Scanner.GetStartPos() };
    template.Head = ParseTemplateHead();
    Debug.Assert(template.Head.Kind == SyntaxKind.TemplateHead,
  ...
  private TemplateSpan ParseTemplateSpan()
  {
    ...
    {
      span.Literal = (TemplateTail)ParseExpectedToken<TemplateTail>(
  ...
  private ILiteralExpression? ParseLiteralNode(bool? internName = null)
  {
    var t = Token();
    if (t == SyntaxKind.StringLiteral)
      return (ILiteralExpression)ParseLiteralLikeNode(new StringLiteral(), internName == true);
    if (t == SyntaxKind.RegularExpressionLiteral)
      return (ILiteralExpression)ParseLiteralLikeNode(new RegularExpressionLiteral(),
        internName == true);
    if (t == SyntaxKind.NoSubstitutionTemplateLiteral)
      return (ILiteralExpression)ParseLiteralLikeNode(new NoSubstitutionTemplateLiteral(),
        internName == true);
    if (t == SyntaxKind.NumericLiteral)
      return (ILiteralExpression)ParseLiteralLikeNode(new NumericLiteral(), internName == true);
    throw new NotSupportedException("parseLiteralNode");
    //return parseLiteralLikeNode(token(), internName == true);
  }

  private TemplateHead ParseTemplateHead()
  {
    var t = Token();
  ...
  private
    /*TemplateMiddle | TemplateTail*/ ILiteralLikeNode ParseTemplateMiddleOrTemplateTail()
  {
    var t = Token();
  ...
  private ILiteralLikeNode ParseLiteralLikeNode( /*SyntaxKind kind*/ ILiteralLikeNode node,
    bool internName)
  {
    node.Pos = Scanner.GetStartPos();
    ...
    if (Scanner.HasExtendedUnicodeEscape()) node.HasExtendedUnicodeEscape = true;
  ...
  private TypeReferenceNode ParseTypeReference()
  {
    ...
    if (!Scanner.HasPrecedingLineBreak() && Token() == SyntaxKind.LessThanToken)
      node.TypeArguments = ParseBracketedList(TsTypes.ParsingContext.TypeArguments, ParseType,
        SyntaxKind.LessThanToken, SyntaxKind.GreaterThanToken);
    return FinishNode(node);
  }

  private TypePredicateNode ParseThisTypePredicate(ThisTypeNode lhs)
  {
    NextToken();
  ...
  private ThisTypeNode ParseThisTypeNode()
  {
  ...
  private TypeQueryNode ParseTypeQuery()
  {
  ...
  private TypeParameterDeclaration ParseTypeParameter()
  {
    ...
    if (ParseOptional(SyntaxKind.EqualsToken)) node.Default = ParseType();
    return FinishNode(node);
  }

  private NodeArray<TypeParameterDeclaration> ParseTypeParameters()
  {
    if (Token() == SyntaxKind.LessThanToken)
      return ParseBracketedList(TsTypes.ParsingContext.TypeParameters, ParseTypeParameter,
        SyntaxKind.LessThanToken, SyntaxKind.GreaterThanToken);
    return null;
  }
  ...
  {
    return Token() == SyntaxKind.DotDotDotToken || IsIdentifierOrPattern() ||
           IsModifierKind(Token()) ||
           Token() == SyntaxKind.AtToken || Token() == SyntaxKind.ThisKeyword;
  }
  ...
  private void FillSignature(SyntaxKind returnToken, bool
    yieldContext, bool
    awaitContext, bool
    ...
    signature)
  {
    ...
    signature.Parameters =
      ParseParameterList(yieldContext, awaitContext, requireCompleteParameterList);
  ...
  private NodeArray<ParameterDeclaration> ParseParameterList(bool yieldContext, bool awaitContext,
    ...
    ITypeElement? ParseSignatureMember(SyntaxKind kind)
  {
    ...
    if (kind == SyntaxKind.ConstructSignature)
    {
      ...
      FillSignature(SyntaxKind.ColonToken, /*yieldContext*/ false, /*awaitContext*/
        false, /*requireCompleteParameterList*/ false, node);
  ...
  {
    if (Token() != SyntaxKind.OpenBracketToken) return false;
  ...
  private IndexSignatureDeclaration? ParseIndexSignatureDeclaration(int fullStart,
    ...
    NodeArray<Modifier> modifiers)
  {
    ...
    node.Parameters = ParseBracketedList(TsTypes.ParsingContext.Parameters, ParseParameter,
  ...
  private
    ...
    ITypeElement? ParsePropertyOrMethodSignature(int fullStart, NodeArray<Modifier> modifiers)
  {
    ...
    if (Token() == SyntaxKind.OpenParenToken || Token() == SyntaxKind.LessThanToken)
    {
      ...
      // [Yield] nor [Await]
      FillSignature(SyntaxKind.ColonToken, /*yieldContext*/ false, /*awaitContext*/
        false, /*requireCompleteParameterList*/ false, method);
      ParseTypeMemberSemicolon();
  ...
  private bool IsStartOfMappedType()
  {
    NextToken();
  ...
  private TypeParameterDeclaration ParseMappedTypeParameter()
  {
    var node = new TypeParameterDeclaration { Pos = Scanner.GetStartPos() };
    node.Name = ParseIdentifier();
  ...
  private MappedTypeNode ParseMappedType()
  {
    var node = new MappedTypeNode { Pos = Scanner.GetStartPos() };
    ParseExpected(SyntaxKind.OpenBraceToken);
    node.ReadonlyToken =
      (ReadonlyToken)ParseOptionalToken<ReadonlyToken>(SyntaxKind.ReadonlyKeyword);
    ParseExpected(SyntaxKind.OpenBracketToken);
  ...
  private TupleTypeNode ParseTupleType()
  {
    ...
    node.ElementTypes = ParseBracketedList(TsTypes.ParsingContext.TupleElementTypes, ParseType,
      SyntaxKind.OpenBracketToken, SyntaxKind.CloseBracketToken);
    return FinishNode(node);
  }

  private ParenthesizedTypeNode ParseParenthesizedType()
  {
  ...
  private IFunctionOrConstructorTypeNode? ParseFunctionOrConstructorType(SyntaxKind kind)
  {
    ...
    if (kind == SyntaxKind.ConstructorType) ParseExpected(SyntaxKind.NewKeyword);
    FillSignature(SyntaxKind.EqualsGreaterThanToken, /*yieldContext*/ false, /*awaitContext*/
      false, /*requireCompleteParameterList*/ false, node);
    return FinishNode(node);
  }

  private TypeNode ParseKeywordAndNoDot()
  {
    ...
    switch (Token())
    {
      ...
      case SyntaxKind.MinusToken:
        return LookAhead(NextTokenIsNumericLiteral)
          ? (ITypeNode)ParseLiteralTypeNode()
          : ParseTypeReference();
  ...
  private
    /*MappedTypeNode*/ TypeOperatorNode ParseTypeOperator(SyntaxKind /*.KeyOfKeyword*/ @operator)
  {
  ...
  private ITypeNode ParseTypeOperatorOrHigher()
  {
    switch (Token())
    {
  ...
  private ITypeNode? ParseUnionOrIntersectionType(
    SyntaxKind /*.UnionType | SyntaxKind.IntersectionType*/ kind,
    Func<ITypeNode> parseConstituentType,
    SyntaxKind /*.BarToken | SyntaxKind.AmpersandToken*/ @operator)
  {
    ParseOptional(@operator);
    var type = parseConstituentType();
    if (Token() == @operator)
    {
      NodeArray<ITypeNode?> types = CreateList<ITypeNode>(); //[type], type.pos);
      types.Pos = type.Pos;
      types.Add(type);
      while (ParseOptional(@operator)) types.Add(parseConstituentType());
      types.End = GetNodeEnd();
      var node = kind == SyntaxKind.UnionType
        ? (IUnionOrIntersectionTypeNode)new UnionTypeNode { Kind = kind, Pos = type.Pos }
        : kind == SyntaxKind.IntersectionType
          ? new IntersectionTypeNode { Kind = kind, Pos = type.Pos }
  ...
  {
    return ParseUnionOrIntersectionType(SyntaxKind.IntersectionType, ParseTypeOperatorOrHigher,
      SyntaxKind.AmpersandToken);
  }

  private ITypeNode? ParseUnionTypeOrHigher()
  {
    return ParseUnionOrIntersectionType(SyntaxKind.UnionType, ParseIntersectionTypeOrHigher,
      SyntaxKind.BarToken);
  }
  ...
  private ITypeNode ParseTypeOrTypePredicate()
  {
  ...
  private Identifier? ParseTypePredicatePrefix()
  {
  ...
  {
    if (IsStartOfFunctionType()) return ParseFunctionOrConstructorType(SyntaxKind.FunctionType);
  ...
  public IExpression? ParseExpression()
  {
    ...
    while ((operatorToken = ( /*BinaryOperator*/
             Token)ParseOptionalToken< /*BinaryOperator*/Token>(SyntaxKind.CommaToken)) !=
           null)
  ...
  private IExpression? ParseInitializer(bool inParameter)
  {
    if (Token() != SyntaxKind.EqualsToken)
      if (Scanner.HasPrecedingLineBreak() ||
          (inParameter && Token() == SyntaxKind.OpenBraceToken) ||
          !IsStartOfExpression())
  ...
  private IExpression? ParseAssignmentExpressionOrHigher()
  {
    ...
    if (arrowExpression != null) return arrowExpression;
    ...
    if (IsLeftHandSideExpression(expr) && IsAssignmentOperator(ReScanGreaterToken()))
      return MakeBinaryExpression(expr, ParseTokenNode< /*BinaryOperator*/Token>(Token()),
  ...
  private YieldExpression? ParseYieldExpression()
  {
    ...
    {
      node.AsteriskToken =
        (AsteriskToken)ParseOptionalToken<AsteriskToken>(SyntaxKind.AsteriskToken);
  ...
  private ArrowFunction? ParseSimpleArrowFunctionExpression(Identifier? identifier,
    NodeArray<Modifier> asyncModifier = null)
  {
    ...
    else
    {
      ...
      {
        Pos = identifier.Pos
      }; // (ArrowFunction)createNode(SyntaxKind.ArrowFunction, identifier.pos);
    }
    ...
    node.EqualsGreaterThanToken =
      (EqualsGreaterThanToken)ParseExpectedToken<EqualsGreaterThanToken>(
        SyntaxKind.EqualsGreaterThanToken, /*reportAtCurrentPosition*/ false,
        Diagnostics._0_expected, "=>");
    node.Body = ParseArrowFunctionExpressionBody( /*isAsync*/ /*!!*/asyncModifier?.Any() == true);
    return AddJsDocComment(FinishNode(node));
  }

  private ArrowFunction TryParseParenthesizedArrowFunctionExpression()
  {
  ...
  private Tristate IsParenthesizedArrowFunctionExpressionWorker()
  {
    ...
    if (first == SyntaxKind.OpenParenToken)
    {
      ...
      if (second == SyntaxKind.DotDotDotToken) return Tristate.True;
    ...
    if (SourceFile.LanguageVariant == LanguageVariant.Jsx)
    {
      var isArrowFunctionInJsx = LookAhead(() =>
      {
        ...
        if (third == SyntaxKind.CommaToken) return true;
        return false;
      });
  ...
  private ArrowFunction? ParsePossibleParenthesizedArrowFunctionExpressionHead()
  {
    return ParseParenthesizedArrowFunctionExpressionHead( /*allowAmbiguity*/ false);
  }

  private ArrowFunction? TryParseAsyncSimpleArrowFunctionExpression()
  {
    if (Token() == SyntaxKind.AsyncKeyword)
    {
      var isUnParenthesizedAsyncArrowFunction =
  ...
  private Tristate IsUnParenthesizedAsyncArrowFunctionWorker()
  {
    if (Token() == SyntaxKind.AsyncKeyword)
    {
      NextToken();
      if (Scanner.HasPrecedingLineBreak() || Token() == SyntaxKind.EqualsGreaterThanToken)
        return Tristate.False;
      var expr = ParseBinaryExpressionOrHigher( /*precedence*/ 0);
      if (!Scanner.HasPrecedingLineBreak() && expr.Kind == SyntaxKind.Identifier &&
          Token() == SyntaxKind.EqualsGreaterThanToken) return Tristate.True;
    }

    return Tristate.False;
  }

  private ArrowFunction? ParseParenthesizedArrowFunctionExpressionHead(bool allowAmbiguity)
  {
    ...
    // close paren.
    FillSignature(SyntaxKind.ColonToken, /*yieldContext*/ false, /*awaitContext*/
      isAsync, /*requireCompleteParameterList*/ !allowAmbiguity, node);
  ...
  private
    /*Block | Expression*/ IBlockOrExpression? ParseArrowFunctionExpressionBody(bool isAsync)
  {
    if (Token() == SyntaxKind.OpenBraceToken)
      return ParseFunctionBlock( /*allowYield*/false, /*allowAwait*/
        isAsync, /*ignoreMissingOpenBrace*/
        false);
    if (Token() != SyntaxKind.SemicolonToken &&
        Token() != SyntaxKind.FunctionKeyword &&
        Token() != SyntaxKind.ClassKeyword &&
  ...
  private IExpression? ParseConditionalExpressionRest(IExpression? leftOperand)
  {
    var questionToken =
      (QuestionToken)ParseOptionalToken<QuestionToken>(SyntaxKind.QuestionToken);
    if (questionToken == null) return leftOperand;
    ...
    node.ColonToken = (ColonToken)ParseExpectedToken<ColonToken>(
      SyntaxKind.ColonToken, /*reportAtCurrentPosition*/ false,
  ...
  private bool IsInOrOfKeyword(SyntaxKind t)
  {
  ...
  private IExpression? ParseBinaryExpressionRest(int precedence, IExpression? leftOperand)
  {
    while (true)
    {
      ...
      if (Token() == SyntaxKind.InKeyword && InDisallowInContext()) break;
      ...
      {
        leftOperand = MakeBinaryExpression(leftOperand,
          ParseTokenNode< /*BinaryOperator*/Token>(Token()),
  ...
  {
    if (InDisallowInContext() && Token() == SyntaxKind.InKeyword) return false;
  ...
  private BinaryExpression? MakeBinaryExpression(IExpression? left, /*BinaryOperator*/
    Token operatorToken,
    IExpression? right)
  {
  ...
  private AsExpression? MakeAsExpression(IExpression? left, ITypeNode right)
  {
  ...
  private PrefixUnaryExpression? ParsePrefixUnaryExpression()
  {
  ...
  private DeleteExpression? ParseDeleteExpression()
  {
  ...
  private TypeOfExpression? ParseTypeOfExpression()
  {
  ...
  private VoidExpression? ParseVoidExpression()
  {
  ...
  private IExpression? ParseUnaryExpressionOrHigher()
  {
    ...
    if (Token() == SyntaxKind.AsteriskAsteriskToken)
    {
      ...
      if (simpleUnaryExpression.Kind == SyntaxKind.TypeAssertionExpression)
        ParseErrorAtPosition(start, (simpleUnaryExpression.End ?? 0) - start,
          Diagnostics
  ...
  private
    /*Unary*/ IExpression? ParseSimpleUnaryExpression()
  {
    switch (Token())
    {
      case SyntaxKind.PlusToken:
  ...
  private bool IsUpdateExpression()
  {
    switch (Token())
    {
      case SyntaxKind.PlusToken:
  ...
  private
    /*Increment*/ IExpression? ParseIncrementExpression()
  {
  ...
  private
    /*LeftHandSideExpression*/ IExpression? ParseLeftHandSideExpressionOrHigher()
  {
  ...
  private IMemberExpression? ParseMemberExpressionOrHigher()
  {
  ...
  private IMemberExpression? ParseSuperExpression()
  {
    ...
    if (Token() == SyntaxKind.OpenParenToken || Token() == SyntaxKind.DotToken ||
        Token() == SyntaxKind.OpenBracketToken) return expression;
    var node = new PropertyAccessExpression { Pos = expression.Pos };
    node.Expression = expression;
    ParseExpectedToken<DotToken>(SyntaxKind.DotToken, /*reportAtCurrentPosition*/ false,
      Diagnostics.super_must_be_followed_by_an_argument_list_or_member_access);
    node.Name = ParseRightSideOfDot( /*allowIdentifierNames*/ true);
    return FinishNode(node);
  }

  private bool TagNamesAreEquivalent(IJsxTagNameExpression lhs, IJsxTagNameExpression? rhs)
  {
    if (lhs.Kind != rhs.Kind) return false;
    if (lhs.Kind == SyntaxKind.Identifier)
      return (lhs as Identifier).Text == (rhs as Identifier).Text;
    if (lhs.Kind == SyntaxKind.ThisKeyword) return true;
  ...
  private
    ...
    PrimaryExpression? ParseJsxElementOrSelfClosingElement(bool inExpressionContext)
  {
    ...
    if (opening.Kind == SyntaxKind.JsxOpeningElement)
    {
      ...
      if (!TagNamesAreEquivalent(tn, node.ClosingElement.TagName))
        ParseErrorAtPosition(node.ClosingElement.Pos ?? 0,
          (node.ClosingElement.End ?? 0) - (node.ClosingElement.Pos ?? 0),
          Diagnostics.Expected_corresponding_JSX_closing_tag_for_0,
          GetTextOfNodeFromSourceText(SourceText, tn));
      var result = FinishNode(node);
      if (inExpressionContext && Token() == SyntaxKind.LessThanToken)
      {
        var invalidElement =
          TryParse(() => ParseJsxElementOrSelfClosingElement( /*inExpressionContext*/true));
        if (invalidElement != null)
        {
          ...
          badNode.OperatorToken = ( /*BinaryOperator*/
            Token)CreateMissingNode<Token>(SyntaxKind.CommaToken, /*reportAtCurrentPosition*/
            false);
  ...
  private NodeArray<IJsxChild> ParseJsxChildren( /*LeftHandSide*/ IExpression openingTagName)
  {
    var result = CreateList<IJsxChild>(); //List<IJsxChild>(); //
    var saveParsingContext = _parsingContext;
    _parsingContext |= 1 << (int)TsTypes.ParsingContext.JsxChildren;
    while (true)
    {
      ...
      if (Token() == SyntaxKind.EndOfFileToken)
      {
        ...
        ParseErrorAtPosition(openingTagName.Pos ?? 0,
          (openingTagName.End ?? 0) - (openingTagName.Pos ?? 0),
          Diagnostics.JSX_element_0_has_no_corresponding_closing_tag,
          GetTextOfNodeFromSourceText(SourceText, openingTagName));

        break;
      }

      if (Token() == SyntaxKind.ConflictMarkerTrivia) break;
  ...
  private JsxAttributes ParseJsxAttributes()
  {
  ...
  private Expression ParseJsxOpeningOrSelfClosingElement(bool inExpressionContext)
  {
  ...
  private IJsxTagNameExpression? ParseJsxElementName()
  {
    ScanJsxIdentifier();
    IJsxTagNameExpression? expression = Token() == SyntaxKind.ThisKeyword
      ? ParseTokenNode<PrimaryExpression>(Token())
      : ParseIdentifierName();
    if (Token() == SyntaxKind.ThisKeyword)
    {
  ...
  private JsxExpression? ParseJsxExpression(bool inExpressionContext)
  {
  ...
  private ObjectLiteralElement ParseJsxAttribute()
  {
    if (Token() == SyntaxKind.OpenBraceToken) return ParseJsxSpreadAttribute();
    ScanJsxIdentifier();
  ...
  private JsxClosingElement ParseJsxClosingElement(bool inExpressionContext)
  {
  ...
  private TypeAssertion? ParseTypeAssertion()
  {
  ...
  private IMemberExpression? ParseMemberExpressionRest( /*LeftHandSideExpression*/
    ...
    while (true)
    {
      ...
      if (!InDecoratorContext() && ParseOptional(SyntaxKind.OpenBracketToken))
      {
        ...
        if (Token() != SyntaxKind.CloseBracketToken)
        {
          ...
          {
            var literal =
              (LiteralExpression)indexedAccess.ArgumentExpression; //(LiteralExpression)
  ...
  private
    ...
    IMemberExpression? ParseCallExpressionRest( /*LeftHandSideExpression*/
      IMemberExpression? expression)
  {
    while (true)
    {
      ...
      if (Token() == SyntaxKind.LessThanToken)
      {
        ...
        if (typeArguments == null) return expression;
  ...
  {
    ParseExpected(SyntaxKind.OpenParenToken);
    var result = ParseDelimitedList(TsTypes.ParsingContext.ArgumentExpressions,
  ...
  {
    if (!ParseOptional(SyntaxKind.LessThanToken)) return null;
  ...
  private IPrimaryExpression? ParsePrimaryExpression()
  {
    switch (Token())
    {
      ...
      case SyntaxKind.SlashToken:
      case SyntaxKind.SlashEqualsToken:
        if (ReScanSlashToken() == SyntaxKind.RegularExpressionLiteral) return ParseLiteralNode();
        break;
      case SyntaxKind.TemplateHead:
  ...
  private Expression? ParseSpreadElement()
  {
  ...
  private IExpression? ParseArgumentOrArrayLiteralElement()
  {
    return Token() == SyntaxKind.DotDotDotToken
      ? ParseSpreadElement()
      : Token() == SyntaxKind.CommaToken
        ? new OmittedExpression
          { Pos = Scanner.GetStartPos() } /*createNode(SyntaxKind.OmittedExpression)*/
        : ParseAssignmentExpressionOrHigher();
  }
  ...
  private ArrayLiteralExpression? ParseArrayLiteralExpression()
  {
    ...
    node.Elements = ParseDelimitedList(TsTypes.ParsingContext.ArrayLiteralMembers,
      ParseArgumentOrArrayLiteralElement);
    ParseExpected(SyntaxKind.CloseBracketToken);
    return FinishNode(node);
  }

  private IAccessorDeclaration? TryParseAccessorDeclaration(int fullStart,
    NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
  ...
  private IObjectLiteralElementLike? ParseObjectLiteralElement()
  {
    var fullStart = Scanner.GetStartPos();
    var dotDotDotToken =
      (DotDotDotToken)ParseOptionalToken<DotDotDotToken>(SyntaxKind.DotDotDotToken);
    ...
    var questionToken =
      (QuestionToken)ParseOptionalToken<QuestionToken>(SyntaxKind.QuestionToken);
    if (asteriskToken != null || Token() == SyntaxKind.OpenParenToken ||
        Token() == SyntaxKind.LessThanToken)
      return ParseMethodDeclaration(fullStart, decorators, modifiers, asteriskToken, propertyName,
        questionToken);
    var isShorthandPropertyAssignment =
      tokenIsIdentifier && (Token() == SyntaxKind.CommaToken ||
                            Token() == SyntaxKind.CloseBraceToken ||
    ...
    if (isShorthandPropertyAssignment)
    {
      ...
      if (equalsToken != null)
      {
        ...
        shorthandDeclaration.ObjectAssignmentInitializer =
  ...
  private ObjectLiteralExpression? ParseObjectLiteralExpression()
  {
    ...
    ParseExpected(SyntaxKind.OpenBraceToken);
    if (Scanner.HasPrecedingLineBreak()) node.MultiLine = true;
    node.Properties = ParseDelimitedList(TsTypes.ParsingContext.ObjectLiteralMembers,
  ...
  private FunctionExpression? ParseFunctionExpression()
  {
    ...
    FillSignature(SyntaxKind.ColonToken, /*yieldContext*/ isGenerator, /*awaitContext*/
      isAsync, /*requireCompleteParameterList*/ false, node);
    node.Body = ParseFunctionBlock( /*allowYield*/ isGenerator, /*allowAwait*/
      isAsync, /*ignoreMissingOpenBrace*/ false);
    if (saveDecoratorContext) SetDecoratorContext( /*val*/true);
    return AddJsDocComment(FinishNode(node));
  }

  private Identifier? ParseOptionalIdentifier()
  {
    return IsIdentifier() ? ParseIdentifier() : null;
  }

  private
    /*NewExpression | MetaProperty*/ IPrimaryExpression? ParseNewExpression()
  {
    var fullStart = Scanner.GetStartPos();
    ParseExpected(SyntaxKind.NewKeyword);
  ...
  private Block? ParseBlock(bool ignoreMissingOpenBrace,
    DiagnosticMessage diagnosticMessage = null)
  {
  ...
  private Block? ParseFunctionBlock(bool allowYield, bool allowAwait, bool ignoreMissingOpenBrace,
    DiagnosticMessage diagnosticMessage = null)
  {
    var savedYieldContext = InYieldContext();
  ...
  private Statement? ParseForOrForInOrForOfStatement()
  {
    var pos = GetNodePos();
    ParseExpected(SyntaxKind.ForKeyword);
    var awaitToken =
      (AwaitKeywordToken)ParseOptionalToken<AwaitKeywordToken>(SyntaxKind.AwaitKeyword);
    ParseExpected(SyntaxKind.OpenParenToken);
  ...
  {
    var node = kind == SyntaxKind.ContinueStatement
      ? (IBreakOrContinueStatement)new ContinueStatement { Pos = Scanner.GetStartPos() }
      : kind == SyntaxKind.BreakStatement
        ? new BreakStatement { Pos = Scanner.GetStartPos() }
        : throw new NotSupportedException("parseBreakOrContinueStatement");
  ...
  private ReturnStatement? ParseReturnStatement()
  {
    ...
    if (!CanParseSemicolon()) node.Expression = AllowInAnd(ParseExpression);
    ParseSemicolon();
    return FinishNode(node);
  }
  ...
  private ICaseOrDefaultClause ParseCaseOrDefaultClause()
  {
    return Token() == SyntaxKind.CaseKeyword
      ? (ICaseOrDefaultClause)ParseCaseClause()
      : ParseDefaultClause();
  }
  ...
  private
    /*ExpressionStatement | LabeledStatement*/ Statement? ParseExpressionOrLabeledStatement()
  {
    var fullStart = Scanner.GetStartPos();
  ...
  private IStatement? ParseStatement()
  {
    switch (Token())
    {
      ...
      case SyntaxKind.VarKeyword:
        return ParseVariableStatement(Scanner.GetStartPos(), /*decorators*/ null, /*modifiers*/
          null);
      case SyntaxKind.LetKeyword:
        if (IsLetDeclaration())
          return ParseVariableStatement(Scanner.GetStartPos(), /*decorators*/ null, /*modifiers*/
        ...
        break;
      case SyntaxKind.FunctionKeyword:
        return ParseFunctionDeclaration(Scanner.GetStartPos(), /*decorators*/ null, /*modifiers*/
          null);
      case SyntaxKind.ClassKeyword:
        return ParseClassDeclaration(Scanner.GetStartPos(), /*decorators*/ null, /*modifiers*/
          null);
      case SyntaxKind.IfKeyword:
    ...
    switch (Token())
    {
      ...
      default:
        if (decorators?.Any() == true || modifiers?.Any() == true)
        {
          ...
          var node = (Statement)CreateMissingNode<Statement>(
  ...
  private Block? ParseFunctionBlockOrSemicolon(bool isGenerator, bool isAsync,
    DiagnosticMessage diagnosticMessage = null)
  {
    ...
    return ParseFunctionBlock(isGenerator, isAsync, /*ignoreMissingOpenBrace*/ false,
      diagnosticMessage);
  }

  private IArrayBindingElement ParseArrayBindingElement()
  {
    if (Token() == SyntaxKind.CommaToken)
      return new OmittedExpression
      {
        Pos = Scanner.GetStartPos()
      }; //(OmittedExpression)createNode(SyntaxKind.OmittedExpression);
  ...
  private IArrayBindingElement ParseObjectBindingElement()
  {
    var node = new BindingElement { Pos = Scanner.GetStartPos() };
    node.DotDotDotToken =
      (DotDotDotToken)ParseOptionalToken<DotDotDotToken>(SyntaxKind.DotDotDotToken);
  ...
  private ObjectBindingPattern? ParseObjectBindingPattern()
  {
    var node = new ObjectBindingPattern { Pos = Scanner.GetStartPos() };
    ParseExpected(SyntaxKind.OpenBraceToken);
    node.Elements = ParseDelimitedList(TsTypes.ParsingContext.ObjectBindingElements,
      ParseObjectBindingElement);
    ParseExpected(SyntaxKind.CloseBraceToken);
    return FinishNode(node);
  }

  private ArrayBindingPattern? ParseArrayBindingPattern()
  {
    var node = new ArrayBindingPattern { Pos = Scanner.GetStartPos() };
    ParseExpected(SyntaxKind.OpenBracketToken);
    node.Elements = ParseDelimitedList(TsTypes.ParsingContext.ArrayBindingElements,
  ...
  private
    /*Identifier | BindingPattern*/ Node? ParseIdentifierOrPattern()
  {
    if (Token() == SyntaxKind.OpenBracketToken) return ParseArrayBindingPattern();
    if (Token() == SyntaxKind.OpenBraceToken) return ParseObjectBindingPattern();
  ...
  private VariableDeclaration ParseVariableDeclaration()
  {
    ...
    node.Name = ParseIdentifierOrPattern();
    node.Type = ParseTypeAnnotation();
    if (!IsInOrOfKeyword(Token())) node.Initializer = ParseInitializer( /*inParameter*/false);
    return FinishNode(node);
  }
  ...
  private IVariableDeclarationList ParseVariableDeclarationList(bool inForStatementInitializer)
  {
    ...
    else
    {
      ...
      node.Declarations =
        ParseDelimitedList(TsTypes.ParsingContext.VariableDeclarations, ParseVariableDeclaration);
  ...
  private VariableStatement? ParseVariableStatement(int fullStart, NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
  ...
  private FunctionDeclaration? ParseFunctionDeclaration(int fullStart,
    NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
  ...
  private ConstructorDeclaration? ParseConstructorDeclaration(int pos,
    NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
  ...
  private MethodDeclaration? ParseMethodDeclaration(int fullStart, NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers, AsteriskToken asteriskToken, IPropertyName? name,
    ...
    DiagnosticMessage diagnosticMessage = null)
  {
    ...
    FillSignature(SyntaxKind.ColonToken, /*yieldContext*/ isGenerator, /*awaitContext*/
      isAsync, /*requireCompleteParameterList*/ false, method);
    method.Body = ParseFunctionBlockOrSemicolon(isGenerator, isAsync, diagnosticMessage);
    return AddJsDocComment(FinishNode(method));
  }

  private ClassElement? ParsePropertyDeclaration(int fullStart, NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers, IPropertyName? name, QuestionToken questionToken)
  {
  ...
  private IClassElement? ParsePropertyOrMethodDeclaration(int fullStart,
    NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
    var asteriskToken =
    ...
    if (asteriskToken != null || Token() == SyntaxKind.OpenParenToken ||
        Token() == SyntaxKind.LessThanToken)
      return ParseMethodDeclaration(fullStart, decorators, modifiers, asteriskToken, name,
        questionToken,
        Diagnostics.or_expected);
  ...
  private IAccessorDeclaration? ParseAccessorDeclaration(SyntaxKind kind, int fullStart,
    NodeArray<Decorator> decorators, NodeArray<Modifier> modifiers)
  {
    var node = kind == SyntaxKind.GetAccessor
      ? (IAccessorDeclaration)new GetAccessorDeclaration { Kind = kind, Pos = fullStart }
      : kind == SyntaxKind.SetAccessor
        ? new SetAccessorDeclaration { Kind = kind, Pos = fullStart }
        : throw new NotSupportedException("parseAccessorDeclaration");
    node.Decorators = decorators;
    node.Modifiers = modifiers;
  ...
  private bool IsClassMemberModifier(SyntaxKind idToken)
  {
    switch (idToken)
    {
      case SyntaxKind.PublicKeyword:
  ...
  private bool IsClassMemberStart()
  {
    ...
    if (Token() == SyntaxKind.AtToken) return true;
    while (IsModifierKind(Token()))
    {
      idToken = Token();
    ...
    if (Token() == SyntaxKind.AsteriskToken) return true;
    if (IsLiteralPropertyName())
    {
      idToken = Token();
      NextToken();
  ...
  private NodeArray<Decorator> ParseDecorators()
  {
    NodeArray<Decorator> decorators = null;
    while (true)
    {
      var decoratorStart = GetNodePos();
      if (!ParseOptional(SyntaxKind.AtToken)) break;
    ...
    if (decorators != null) decorators.End = GetNodeEnd();
    return decorators;
  }

  private NodeArray<Modifier> ParseModifiers(bool? permitInvalidConstAsModifier = null)
  {
    ...
    while (true)
    {
      ...
      var modifier = FinishNode(new Modifier
      {
        Kind = modifierKind, Pos = modifierStart
      }); //(Modifier)createNode(modifierKind, modifierStart)
      if (modifiers == null)
      {
    ...
    if (modifiers != null) modifiers.End = Scanner.GetStartPos();
    return modifiers;
  }

  private NodeArray<Modifier> ParseModifiersForArrowFunction()
  {
  ...
  private IClassElement? ParseClassElement()
  {
    ...
    if (decorators?.Any() == true || modifiers?.Any() == true)
    {
      var name = (Identifier)CreateMissingNode<Identifier>(
        SyntaxKind.Identifier, /*reportAtCurrentPosition*/
  ...
  private ClassDeclaration? ParseClassDeclaration(int fullStart, NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
  ...
  private Identifier? ParseNameOfClassDeclarationOrExpression()
  {
  ...
  private NodeArray<HeritageClause> ParseHeritageClauses()
  {
    if (IsHeritageClause())
  ...
  private HeritageClause ParseHeritageClause()
  {
    var tok = Token();
    if (tok == SyntaxKind.ExtendsKeyword || tok == SyntaxKind.ImplementsKeyword)
    {
      ...
      node.Token = tok;
      NextToken();
      node.Types = ParseDelimitedList(TsTypes.ParsingContext.HeritageClauseElement,
  ...
  private ExpressionWithTypeArguments ParseExpressionWithTypeArguments()
  {
  ...
  private InterfaceDeclaration? ParseInterfaceDeclaration(int fullStart,
    NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
  ...
  private TypeAliasDeclaration? ParseTypeAliasDeclaration(int fullStart,
    NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
  ...
  private EnumDeclaration? ParseEnumDeclaration(int fullStart, NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
  ...
  private ModuleDeclaration? ParseModuleOrNamespaceDeclaration(int fullStart,
    NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers, NodeFlags flags)
  {
    ...
    node.Body = ParseOptional(SyntaxKind.DotToken)
      ? ( /*NamespaceDeclaration*/Node)ParseModuleOrNamespaceDeclaration(
        GetNodePos(), /*decorators*/
        null, /*modifiers*/ null, NodeFlags.NestedNamespace | namespaceFlag)
      : ParseModuleBlock();
    return AddJsDocComment(FinishNode(node));
  }

  private ModuleDeclaration? ParseAmbientExternalModuleDeclaration(int fullStart,
    NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
    var node = new ModuleDeclaration { Pos = fullStart };
  ...
  private ModuleDeclaration? ParseModuleDeclaration(int fullStart, NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
    NodeFlags flags = 0;
    if (Token() == SyntaxKind.GlobalKeyword)
  ...
  private NamespaceExportDeclaration? ParseNamespaceExportDeclaration(int fullStart,
    NodeArray<Decorator> decorators, NodeArray<Modifier> modifiers)
  {
  ...
  private
    ...
    IStatement? ParseImportDeclarationOrImportEqualsDeclaration(int fullStart,
      NodeArray<Decorator> decorators,
      NodeArray<Modifier> modifiers)
  {
  ...
  private ImportEqualsDeclaration? ParseImportEqualsDeclaration(int fullStart,
    NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers, Identifier? identifier)
  {
  ...
  private ImportClause ParseImportClause(Identifier? identifier, int fullStart)
  {
  ...
  private INode? ParseModuleReference()
  {
  ...
  private ExternalModuleReference ParseExternalModuleReference()
  {
  ...
  public INamedImportsOrExports ParseNamedImportsOrExports(SyntaxKind kind)
  {
    if (kind == SyntaxKind.NamedImports)
    {
      ...
      node.Elements = ParseBracketedList(TsTypes.ParsingContext.ImportOrExportSpecifiers,
  ...
  public ImportSpecifier ParseImportSpecifier()
  {
    ...
    if ( /*kind == SyntaxKind.ImportSpecifier && */checkIdentifierIsKeyword)
      ...
      ParseErrorAtPosition(checkIdentifierStart, checkIdentifierEnd - checkIdentifierStart,
  ...
  public ExportDeclaration? ParseExportDeclaration(int fullStart, NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
  ...
  public ExportAssignment? ParseExportAssignment(int fullStart, NodeArray<Decorator> decorators,
    NodeArray<Modifier> modifiers)
  {
  ...
  private void SetExternalModuleIndicator(SourceFile sourceFile)
  {
    sourceFile.ExternalModuleIndicator = sourceFile
      .Statements.FirstOrDefault(node => 
